...
if (isset($_POST['insert_brand'])) {
    ...
    // selcting data from db
    $select_query="Select * from `brands` where brand_title='$brand_title'";
    $result_select=mysqli_query($con,$select_query);
    $num=mysqli_num_rows($result_select);
    if ($num>0) {
        echo "<script> alert('The given brand is already present in the database')</script>";
    }
...
<form action="" method="post" class="mb-2">
    ...
    <div class="input-group w-10 mb-2 m-auto">
        <input type="submit" class="bg-info border-0 p-2 my-3" name="insert_brand" value="Insert Brand" aria-label="Username" aria-describedby="basic-addon1">
    </div>
</form>